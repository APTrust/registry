...
func (gf *GenericFile) saveEventsTx(tx *pg.Tx) error {
	for _, event := range gf.PremisEvents {
		...
		if validationErr != nil {
			common.Context().Log.Error().Msgf("GenericFile save failed on validation of event (%s) - %s. Error: %s", gf.Identifier, common.ConvertEventTypeToString(event.EventType), validationErr.Error())
			return validationErr
		}
		// Premis events can only be inserted, not updated.
		_, err := tx.Model(event).Insert()
		if err != nil {
			common.Context().Log.Error().Msgf("GenericFile batch insertion failed on insert of event (%s) - %s. Error: %s", gf.Identifier, common.ConvertEventTypeToString(event.EventType), err.Error())
			return err
		}
	}
	return nil
