...
func TestEventShow(t *testing.T) {
	...
	instAdminClient.GET("/events/show/42").
		Expect().Status(http.StatusForbidden)
	instUserClient.GET("/events/show/42").
		Expect().Status(http.StatusForbidden)
}
...
func TestEventShowXHR(t *testing.T) {
	...
	// Inst 1 users cannot see event belonging to inst 2
	instAdminClient.GET("/events/show_xhr/42").
		Expect().Status(http.StatusForbidden)
	instUserClient.GET("/events/show_xhr/42").
		Expect().Status(http.StatusForbidden)
}
